...
SciMLBase.has_init(opt::AdaptiveNonMonotoneTR) = true
SciMLBase.allowscallback(opt::AdaptiveNonMonotoneTR) = true
SciMLBase.requiresgradient(opt::AdaptiveNonMonotoneTR) = true
SciMLBase.allowsbounds(opt::AdaptiveNonMonotoneTR) = true
SciMLBase.allowsfg(opt::AdaptiveNonMonotoneTR) = true


function AdaptiveNonMonotoneTR(; μ0 = 0.1, c = 0.5,ξ0 = 0.85,θmin=0.95)
    AdaptiveNonMonotoneTR(μ0,c,ξ0,θmin)
end


function SciMLBase.__init(prob::OptimizationProblem, opt::AdaptiveNonMonotoneTR;
    ...
    return OptimizationCache(prob, opt; maxiters, callback, progress,
                             save_best, initial_hessian ,kwargs...)
end
...
function SciMLBase.__solve(cache::OptimizationCache{O}) where {O <: AdaptiveNonMonotoneTR}
    uType = eltype(cache.reinit_cache.u0)
    c = uType(cache.opt.c)
    μ0 = uType(cache.opt.μ0)
    ...
    opt_state = OptimizationBase.OptimizationState(
        ; iter=0, u = x, objective = val, grad = gr, hess = H, original = (;Δ=nothing,D,ρ), p = cache.p)
    ...
    @inbounds for iter in 1:maxiters
        ...
        while true
            ...
            ρ = if Δm > 0
                (D-cand_val-bound_augmentation)/(Δm)
            else
                -one(uType)
            end
