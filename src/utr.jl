...
# From 10.1007/s10915-025-03154-y
struct UniversalTR 
    η::Float64
    ξ::Float64
    γ1::Float64
...
SciMLBase.has_init(opt::UniversalTR) = true
SciMLBase.allowscallback(opt::UniversalTR) = true
SciMLBase.requiresgradient(opt::UniversalTR) = true
SciMLBase.allowsbounds(opt::UniversalTR) = true
SciMLBase.allowsfg(opt::UniversalTR) = true


function UniversalTR(; 
                     η = 1/64,ξ = 0.5, γ1 = 2., γ2 = 2.,
                     ρmin = 1e-8, θmin = 0.95)

    UniversalTR(η,ξ,γ1,γ2,ρmin,θmin)
end


function SciMLBase.__init(prob::OptimizationProblem, opt::UniversalTR;
    ...
    return OptimizationCache(prob, opt; maxiters, callback, progress,
                             initial_radius, initial_hessian ,kwargs...)
end
...
function _check_conditions(val,gr,cand_val,cand_gr,H,η,ξ,ρ)
    δ = cand_val-val
    monotone = δ <= 0

    δ <= -η/ρ*norm(gr)^(3/2) || norm(cand_gr) <= ξ*norm(gr)
...
function SciMLBase.__solve(cache::OptimizationCache{O}) where {O <: UniversalTR}
    ...
    @inbounds for iter in 1:maxiters
        ...
        while true
            ...
            if _check_conditions(val,gr,cand_val,cand_gr,H)
                break
            end
            ρ = γ1*ρ
        end
